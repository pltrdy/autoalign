...
def read_ctm(ctm_path):
    with open(ctm_path) as f:
        lines = f.readlines()
    lines = [_.split('\t')[4] for _ in lines]
    lines = [_ if not _.startswith('<start=') else "\n\n" for _ in lines]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run texttiling on CTM file")
    parser.add_argument('--file', '-f', help="Path to CTM file")
